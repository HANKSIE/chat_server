...
class GroupRepository
{
    ...
    public function recentContactPaginate($userID, $isOneToOne, $perPage)
    {
        ...
        $paginate = MessageRead::with($isOneToOne ?
            [
                'latestMessage.group.members' => function ($query) use ($userID) {
                    $query->where('group_members.user_id', '!=', $userID);
                },
            ] : 'latestMessage.group')
            ->whereHas('group', function ($query) use ($isOneToOne) {
                $query->where('is_one_to_one', $isOneToOne);
            })
            ->whereHas('group.members', function ($query) use ($userID) {
                $query->where(['group_members.user_id' => $userID, 'group_members.deleted_at' => null]);
            })
            ->where('message_read.user_id', $userID)
            ->where('message_read.latest_message_id', '!=', null)
        ...
        $paginate->through(function ($messageRead) {
            return [
                'message' => $messageRead->latestMessage,
